...
// Lifted from the old src/app/page.js's `Home` component (the tab-switching
// dashboard shell) when the dashboard moved to real routes — every piece of
// cross-page state (auth, cart, orders, menu, the one shared Socket.IO
// connection) that used to live in that single component now lives here,
// read via useDashboard() by every route under src/app/(dashboard)/**.
...
export function useDashboardState() {
  ...
  // --- Auth state ---
  // Starts `undefined` ("haven't checked localStorage yet") rather than
  // eagerly reading localStorage in the initializer — that lazy-useState
  // approach (what the old tab-based Home used) renders LoginPage during
  // SSR (no window) and the real dashboard during hydration for a returning
  // logged-in user, a genuine hydration mismatch. Resolving it inside an
  // effect instead means the very first client render also has nothing to
  // hydrate against yet, matching the server. src/app/admin/layout.js
  // already uses this three-state (undefined/''/token) pattern.
  ...
  useEffect(() => {
    ...
    setAuthName(localStorage.getItem('userName') || '');
  ...
  // pattern as handleStatusChange/handleClaimOrder above so the Orders
  // route stays in sync without a second fetch.
