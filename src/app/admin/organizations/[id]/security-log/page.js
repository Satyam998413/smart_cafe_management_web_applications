...
import { ShieldAlert, ChevronLeft, ChevronRight, RotateCcw } from 'lucide-react';
...
// This org's password-reset history (plan Phase 1e) — audit_log filtered
// to action=password_reset, reusing the same GET /api/admin/audit-log
// route the global /admin/activity feed and this org's /history page also
// read from, just with a different action filter baked in.
export default function OrgSecurityLogPage() {
  ...
  const load = async (targetPage = 1) => {
    ...
    setError('');
    try {
      const params = new URLSearchParams({ orgId: org.id, action: 'password_reset', page: String(targetPage), limit: String(PAGE_SIZE) });
      const res = await apiFetch(`/admin/audit-log?${params.toString()}`);
      const data = await res.json();
      if (!res.ok) {
        setError(data.message || 'Could not load the security log.');
        return;
      }
      ...
      console.error('Failed to load security log:', e);
    ...
    <div>
      {loading ? (
        ...
        >
          ...
          No password resets recorded for this organization.
        </div>
      ) : (
        <>
                ...
                <tbody>
                  {entries.map((entry) => (
                    <motion.tr key={entry.id} variants={rowVariants}>
                      ...
                      <td style={{ color: 'var(--text-secondary)' }}>{entry.targetId || '—'}</td>
                    </motion.tr>
                  ))}
                </tbody>
              </table>
